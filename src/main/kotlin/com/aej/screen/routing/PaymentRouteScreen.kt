...
object PaymentRouteScreen {
    ...
    suspend fun createPayment(applicationCall: ApplicationCall) = applicationCall.run {
        ...
        val newTransaction = transaction.apply {
            paymentTransaction.statusPayment = paymentData.statusPayment
        }
    ...
    suspend fun getCurrentPayment(applicationCall: ApplicationCall) = applicationCall.run {
        val user = User.fromToken(request, userRepository)
        val payment = paymentRepository.getAllPayment(user.id)
            .filter {
                it.statusPayment == Transaction.StatusPayment.WAITING || it.statusPayment == Transaction.StatusPayment.PENDING
            }
